...
let notice_pass = ref('')
// 获取消息参数，一些限制
let get_message_arg = ref({
  get_count: 10,
  type: GetMsgWay.NORMAL,
  chatroom: props.page_name,
})
// 获取评论参数，一些限制
let get_comment_arg = ref({
  get_count: 10,
  msg_id: 1,
  type: 0,
  chatroom: props.page_name,
...
// 进入页面前的操作，检查是否需要密码
// 修改 ChatPage.vue 中的 before_start 函数
async function before_start() {
  if (props.type_ === RoomType.PRIVATE) {
    // 1. 检查本地缓存是否已有权进入
    if (store.get(props.page_name).value === 'ok') {
      start()
      return // 已验证，直接开始并退出函数
    }

    // 2. 弹出密码输入框
    let input = prompt("请输入进入密码：")

    // 重点修复：点击“取消”时 input 为 null
    if (input === null) {
      alert('已取消进入')
      route.push('/')
      return // 关键：拦截逻辑，不再向下执行
    }

    // 3. 验证密码
    const verifyResult = verify_pass(input, 'chatroom', props.page_name)
    
    // 假设 verify_pass 成功时返回 true，失败时返回错误字符串
    if (verifyResult === true) {
      // 设置进入密码 localStorage，三天失效
      store.set(props.page_name, 'ok', Date.now() + 1000 * 60 * 60 * 24 * 3)
      start()
    } else {
      // 验证失败的情况
      alert(verifyResult || '密码错误')
      alert('即将跳转主页')
      route.push('/')
      // 这里不需要 return，因为函数已经到头了，但为了严谨可以加上
    }
  } else {
    // 公共房间直接开始
    start()
...
async function get_ip_address() {
  await axios.get('/server/get_user_ip')
    .then(response => {
      let data = response.data
      console.log(data)
      user.value.ip = data.ip
      user.value.address = data.pro
      user.value.address_logs = data.addr
    })
  is_blacklist.value = await verify_blacklist(user.value.ip)
  // 访问记录插入
...
id: 消息ID
chatroom: 消息所属聊天室名称
type: 消息类型（0为普通消息，1为公告）
sender: 发送者昵称
content: 消息内容
imgSrc: 图片数据（字符串形式，逗号分隔）
time: 发送时间
ip: 发送者IP
address: 发送者地址
comment_count: 发送者评论数
*/
function get_message(type) {
  get_message_arg.value.type = type
  // type为2时，获取最新消息
  if (type === GetMsgWay.START) get_message_arg.value.get_count = 10
  axios.post("/server/get_message", get_message_arg.value).then(response => {
    let data = response.data
    // 获取数据并插入res中
    // 图片数据处理，将图片数据分割为数组
    for (let i = 0; i < data.length; i++) {
      data[i].imgSrc = String(data[i].imgSrc).split(',').filter(item => item && item !== 'null' && item !== '');
    }
...
function get_notice(type) {
  ...
  // type为2时，获取最新消息
  if (type === GetMsgWay.START) get_message_arg.value.get_count = 10
  axios.post("/server/get_notice", get_message_arg.value).then(response => {
    let data = response.data
    // 预处理数据
    for (let i = 0; i < data.length; i++) {
      // 分割图片路径同时防止空字符串
...
function get_comments(type) {
  get_comment_arg.value.type = type
  // type为2时，获取最新消息
  if (type === GetMsgWay.START) get_comment_arg.value.get_count = 10
  axios.post("/server/get_comments", get_comment_arg.value).then(response => {
    let data = response.data
    // 获取数据并插入res中
    // 图片数据处理，将图片数据分割为数组
    for (let i = 0; i < data.length; i++) {
      data[i].imgSrc = String(data[i].imgSrc).split(',');
    }
    if (type === GetMsgWay.START) {
...
function get_more() {
  ...
  if (documentHeight - windowHeight - scrollTop - 1 <= 0) {
    ...
    // 判断请求内容是普通信息还是公告（和按钮功能有关）
    if (choose.value) {
      get_notice(GetMsgWay.MORE)
      isLoading.value = false; // 请求结束后解锁
      get_message_arg.value.get_count += 10
      return
    }
...
async function get_more_c() {
  ...
  if (scrollTop + clientHeight + 1 >= scrollHeight) {
    ...
    // limit增加
    get_comment_arg.value.get_count += 10
  }
}
...
<template>
  ...
  <el-dialog class="dialog" destroy-on-close @close="get_comment_arg.get_count = 10" v-model="dialogVisible"
    :width="dialogWidth">
    <nav id="nav">
    ...
    <div id="comment-show" @scroll="get_more_c">
      <div class="comment-item" v-for="(item, index) in commentList" :key="index">
        ...
        <p v-if="item.type === 1" style="color:red" class="content">{{ item.content }}</p>
        <p v-else class="content">{{ item.content }}</p>
        <el-image @click="isShowPics = true" class="msgImg" v-for="(img, index) in item.imgSrc" :key="index"
          v-if="item.imgSrc[0] !== 'null' && item.imgSrc[0] !== ''" :src="'/src/assets/upload/' + img"
          alt="图片"></el-image>
        <!--        <el-image-viewer :src-list="item.imgSrc" :z-index="1000"></el-image-viewer>-->
        <div class="otherInfo">
          <span class="small address">地址: {{ item.address }}</span>
          <span class="small date">{{ item.datatime }}</span><br />
        </div>
      </div>
    </div>
    <hr />
    ...
    <div id="wrap">
      ...
      <div class="item" v-for="(item, index) in messageList" :key="index">
        ...
        <div class="otherInfo">
          ...
          <span class="small address">地址: {{ item.address }}</span>
          <!-- 发送时间 -->
          <span class="small date">{{ item.datetime }}</span><br />
        </div>
        <!-- 评论按钮，仅在非主页(type_ !== 0)时显示 -->
        <!-- 点击评论按钮时，设置msg_id并打开评论弹窗 -->
        <!-- msg_id均设置为item.id，方便评论弹窗获取评论数据-->
        <el-button v-if="props.type_ !== 0" class="comment" link
          @click="user.msg_id = item.id; get_comment_arg.msg_id = item.id; dialogVisible = true; get_comments(GetMsgWay.START);">
